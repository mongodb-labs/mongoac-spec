    ...
    fs::write(
        ...
        format!(
            "// Generated by src/libmongoac/build.rs.\n\
             ...
             pub const MONGOAC_BUILD_PLATFORM: &str = \"{platform}\";\n",
            major = version[0],
            minor = version[1],
            patch = version[2],
            prerelease = prerelease,
            full = full,
            platform = format!("{}{}", build_type, link_type),
        ),
    )
...
fn generate_crate_header(path: &Path, src_root: &Path, include_dir: &Path) {
    ...
    config.autogen_warning = Some("// Generated by src/libmongoac/build.rs.".to_owned());
    ...
    config.function.prefix = Some("MONGOAC_API".to_owned());
...
fn collect_files(dir: &Path, files: &mut Vec<std::path::PathBuf>) {
    for entry in std::fs::read_dir(dir).expect("failed to read directory") {
        ...
        if path.is_dir() {
            if path.file_name().map_or(false, |name| name == "private") {
                continue;
            }
...
fn link_with_bson() {
    fn emit_linkage(env_var: &str, is_shared: bool) {
        ...
        let dir = path
            .parent()
            .expect(&format!("{} has no parent directory", env_var));
    ...
    if let Ok(_) = std::env::var("MONGOAC_BSON_SHARED_LIBRARY_FILENAME") {
        emit_linkage("MONGOAC_BSON_SHARED_LIBRARY_FILENAME", true);
    } else if let Ok(_) = std::env::var("MONGOAC_BSON_STATIC_LIBRARY_FILENAME") {
