...
TEST_CASE("create_collection_options_new_from_bson", "[mongoac][create_collection_options]")
{
   auto const error = make_owning_ptr(mongoac_error_new(), &mongoac_error_destroy);

   SECTION("null bson")
   {
      auto const opts = make_owning_ptr(mongoac_create_collection_options_new_from_bson({}, error),
                                        &mongoac_create_collection_options_destroy);
   ...
   SECTION("invalid")
   {
      ...
      auto const opts = mongoac_create_collection_options_new_from_bson({data, sizeof(data)}, error);
   ...
   SECTION("valid")
   {
      ...
      auto const opts = make_owning_ptr(mongoac_create_collection_options_new_from_bson(make_bson_view(bson), error),
                                        &mongoac_create_collection_options_destroy);
      CHECK_MONGOAC_OK(error);
      REQUIRE(opts != nullptr);
   }
