...
TEST_CASE("set_comment", "[mongoac][insert_many_options]")
{
   ...
   SECTION("null handle")
   {
      ...
      mongoac_insert_many_options_set_comment(nullptr, make_bson_view(comment), error);
      REQUIRE_MONGOAC_INVALID_ARGUMENT(error);
   }

   SECTION("null bson clears")
   {
      mongoac_insert_many_options_set_comment(opts, {}, error);
   ...
   SECTION("invalid")
   {
      ...
      mongoac_insert_many_options_set_comment(opts, {data, sizeof(data)}, error);
   ...
   {
      auto const comment = make_owning_ptr(bson_from_json(R"({"x": 1})"), &bson_destroy);
      mongoac_insert_many_options_set_comment(opts, make_bson_view(comment), error);
      CHECK_MONGOAC_OK(error);
   }
}
