...
#
# mongoac_catch_discover_tests helper script.
#
# This script is invoked in CMake script mode (-P) by mongoac_catch_discover_tests()
# to parse Catch2's JSON test listing and generate CTest entries with tag-based
# unique names.
#
# For a test case "name" with tags [a][b][c], the CTest test name is:
#     <prefix>a/b/c/name<suffix>
# Spaces in test names are replaced with underscores.
# Special tags (beginning with a non-alphanumeric character, e.g. [.], [!throws])
# are ignored when constructing the name. The "mongoac" tag is also excluded
# because the "mongoac/" prefix already identifies these tests.
...
function(mongoac_catch_discover_tests_impl)
  ...
  foreach(idx RANGE ${num_tests})
    ...
    # Build CTest test name from plain_name + non-special tags.
    # Special tags begin with a non-alphanumeric character (e.g. [.], [!throws]).
    ...
    if(num_tags GREATER_EQUAL "0")
      foreach(tag_idx RANGE ${num_tags})
        ...
        # Check if this is a special tag (first character not alphanumeric)
        string(SUBSTRING "${a_tag}" 0 1 first_char)
        if(first_char MATCHES "^[a-zA-Z0-9]$")
          # Exclude the "mongoac" tag from the CTest name; it is redundant with the
          # TEST_PREFIX "mongoac/" already applied by mongoac_catch_discover_tests().
          if(NOT a_tag STREQUAL "mongoac")
            list(APPEND tag_name_parts "${a_tag}")
          endif()
        endif()

        # Build label list (same escaping as CatchAddTests.cmake)
        string(REPLACE ";" "\\;" a_tag "${a_tag}")
        list(APPEND tag_list "${a_tag}")
      endforeach()
    endif()

    # Sort tags alphabetically for deterministic naming
    if(tag_name_parts)
      list(SORT tag_name_parts)
      string(REPLACE ";" "/" tag_prefix "${tag_name_parts}")
      set(ctest_name "${tag_prefix}/${plain_name}")
    else()
